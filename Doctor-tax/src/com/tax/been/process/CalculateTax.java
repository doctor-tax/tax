		...
		if(check.size() != 0 && check.get(0).get("status").equals("c")) {
			...
			for (int i = 0; i < listData.size(); i++) {
				...
				String sql = "SELECT t10.id,t10.hcode,t10.doctor_income,t10.Income,t10.sum_pay_tax,t10.stdate,t10.sum_tax_break,SUM(t10.sum_donate) as 'sum_donate' FROM "
						...
						+ "') t5,(SELECT CASE WHEN ISNULL(COUNT(sum_pay_tax),0)=0 THEN 0 WHEN (SELECT sum_pay_tax FROM pay_tax WHERE tax_period = '"+beforeMonth+"' AND doctor_id='"+id+"') <> 0 THEN (SELECT sum_pay_tax FROM pay_tax WHERE tax_period = '"+beforeMonth+"' AND doctor_id='"+id+"') ELSE 0 END AS 'sum_pay_tax' FROM pay_tax WHERE tax_period = '"+beforeMonth+"' AND doctor_id='"+id+"') t6" + " WHERE t3.doctor_id = '" + id
